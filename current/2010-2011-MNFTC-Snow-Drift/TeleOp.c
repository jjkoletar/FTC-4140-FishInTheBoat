...
void initializeRobot()
{
  ...
  servo[leftGoalHolder] = 206;
  servo[preloadServo] = 255;
  servoChangeRate[bridgeServo] = 20;
  blinks[0] = false;
  blinks[1] = false;
  blinks[2] = false;
  return;
...
void bridgeArm()
{
  //0 = forward
  //4 = back
  if (joystickVal(1, "hat")==0) servo[bridgeServo] = servo[bridgeServo] + 3;
  else if (joystickVal(1, "hat")==4) servo[bridgeServo] = servo[bridgeServo] - 3;

  if (servo[bridgeServo] > 155) servo[bridgeServo] = 155;
  else if (servo[bridgeServo] < 0) servo[bridgeServo] = 0;
}
...
void holdGoalWatcher()
{
  ...
  if (servo[leftGoalHolder] > 207) servo[leftGoalHolder] = 207;
  else if (servo[leftGoalHolder] < 160) servo[leftGoalHolder] = 160;
  // \left goal holder
  //right goal holder
  //range is 0-40
  if (abs(joystickVal(2, "x2")) > deadZone && joystickVal(2, "x2") < 0) servo[rightGoalHolder] = servo[rightGoalHolder] + 3;
  else if (abs(joystickVal(2, "x2")) > deadZone && joystickVal(2, "x2") > 0) servo[rightGoalHolder] = servo[rightGoalHolder] - 3;
  if (servo[rightGoalHolder] > 40) servo[rightGoalHolder] = 40;
  else if (servo[rightGoalHolder] < 0) servo[rightGoalHolder] = 0;
  // \right goal holder
}
